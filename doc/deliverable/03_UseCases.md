...
| Primary actor           | Cittadino registrato e Cittadino non registrato   |
| Supporting actors       | OpenStreetMap (STK-8) |
| Stakeholders' interests | **Cittadino registrato/ non registrato [STK-1, STK-2]**: desidera visualizzare i dettagli della segnalazione.<br> **Comune di Torino [STK-9]**: mostrare correttamente i dettagli di una segnalazione |
...
| Trigger                 | L'operatore riceve aggiornamenti sul campo dall'Ufficio Tecnico (es. i lavori sono iniziati o c'è un ritardo) e deve registrarli a sistema. |
...
| Scope                   | Piattaforma Participium |
| Level                   | User goal            |
| Intention in Context    | Operatore identifica contenuti non conformi (es. dati sensibili, foto inappropriate, insulti) e avvia una segnalazione all’amministratore. |
| Primary actor           | Operatore comunale  |
| Supporting actors       | Amministratore (STK-4), Servizio notifiche (STK-6) |
| Stakeholders' interests | **Cittadino registrato / non registrato [STK-1, STK-2]**: vuole fruire di un ambiente civile e rispettoso della privacy. <br> **Admin [STK-4]**: vuole avere sotto controllo le violazioni per poter prendere provvedimenti. <br> **Operatore Comunale [STK-3]**: vuole strumenti per mantenere il sistema affidabile e conforme al GDPR.|
| Precondition            | L'operatore è autenticato e sta visualizzando una segnalazione o un messaggio contenente materiale sospetto. |
| Minimum guarantees      | La segnalazione di moderazione non altera lo stato visibile al cittadino fino all'intervento dell'Admin.|
| Success guarantees      | L'Admin riceve un ticket di moderazione dettagliato nel proprio pannello. |
| Trigger                 | L'operatore rileva un contenuto non conforme (es. volti, targhe, spam) durante la valutazione di una segnalazione o leggendo un messaggio. |
| Main success scenario   | 1. Fornisce motivo e dettagli per rimozione.[FR-24]<br> 2. Il sistema salva la segnalazione interna e invia una notifica all'Admin tramite il servizio notifiche.<br> Il caso d'uso termina con successo.|
| Extensions              | 1a. L'operatore non fornisce dettagli sufficienti nella segnalazione interna.<br>1a.1 Il sistema mostra un errore richiedendo ulteriori verifiche e dettagli prima di procedere [FR-24]. Il flusso riprende dal punto 1.
...
| Scope                   | Piattaforma Participium     |
| Level                   | User goal                   |
| Intention in Context    | Fornire chiarimenti o aggiornamenti diretti al cittadino in risposta a una sua richiesta nella sezione di messaggistica.|
| Primary actor           | Operatore comunale  |
| Supporting actors       | Servizio notifiche (STK-6), Mail server (STK-7) |
...
| Success guarantees      | Il messaggio è recapitato al cittadino, la cronologia della conversazione è aggiornata e il cittadino è stato notificato. |
| Trigger                 | L'operatore legge un messaggio da parte di un cittadino e decide di rispondere. |
| Main success scenario   | 1. L'operatore accede alla sezione messaggistica della segnalazione specifica.<br>2. L'operatore inserisce il testo della risposta.<br>3. L'operatore conferma l'invio.<br>4. Il sistema valida il contenuto e salva il messaggio [FR-16].<br>5. Il sistema invia una notifica push al cittadino segnalante tramite il servizio notifiche [FR-14] e, se abilitata, una notifica email tramite mail server [FR-15].<br>6. Il sistema aggiorna l'interfaccia della chat mostrando il nuovo messaggio.<br>Il caso d'uso termina con successo. |
| Extensions              | - |
...
| Precondition            | L'operatore è autenticato nell'area gestionale riservata. |
| Minimum guarantees      | L'operazione di filtro non altera in alcun modo i dati salvati nel sistema.                          |
| Success guarantees      | L'operatore visualizza i dati aggiornati filtrati secondo i propri criteri di competenza. |
| Trigger                 | -                           |
| Main success scenario   | 1. L'operatore accede alla dashboard principale del sistema.<br>2. Il sistema mostra la tabella completa di tutte le segnalazioni attive [FR-17].<br>3. L'operatore imposta i filtri desiderati (es. "Solo segnalazioni in stato Pending Approval", "Categoria: Buche") [FR-17].<br>4. Il sistema interroga il database e aggiorna la vista mostrando solo i risultati pertinenti.<br>Il caso d'uso termina con successo. |
...
| Precondition            | L'utente in questione è stato segnalato da un operatore per violazione dei termini. |
...
| Trigger                 | L'Admin riceve una segnalazione di moderazione dall'operatore comunale. |
| Main success scenario   | 1. L'Admin accede alla lista degli utenti segnalati per abusi.<br>2. L'Admin revisiona le segnalazioni e le prove caricate (es. foto inappropriate).<br>3. L'Admin banna l'utente.<br>4. Il sistema comunica al Servizio di Autenticazione di revocare i permessi all'utente [FR-23].<br>5. Il sistema disabilita la sessione attiva dell'utente e nasconde le sue segnalazioni correnti dalla vista pubblica [FR-23].<br>6. Il sistema registra l'azione nel log di moderazione.<br>Il caso d'uso termina con successo. |
...
| Minimum guarantees      | L'estrazione dei dati statistici non degrada le performance generali del sistema. |
| Success guarantees      | L'Admin visualizza dashboard interattive con metriche avanzate. |
| Trigger                 | -                           |
| Main success scenario   | 1. L'Admin accede alla sezione dedicata alle statistiche private della piattaforma.<br>2. Il sistema genera automaticamente grafici su: numero di segnalazioni per stato e tipologia, segnalanti più attivi, e distribuzione del top 1%/5% dei segnalanti per tipologia [FR-21].<br>3. L'Admin applica filtri temporali o geografici.<br>4. Il sistema aggiorna i report in tempo reale ricalcolando i dati.<br>5. L'Admin esporta opzionalmente i dati per uso istituzionale [FR-12].<br> Il caso d'uso termina con successo. |
| Extensions              | -                           |
